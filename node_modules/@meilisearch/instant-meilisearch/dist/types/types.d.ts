...
import type { MultiSearchQuery, MultiSearchResult, Config as MeilisearchConfig, Meilisearch as MeilisearchClient } from 'meilisearch';
...
export type { Filter, FacetDistribution, FacetStats as MeiliFacetStats, Config as MeilisearchConfig, } from 'meilisearch';
...
type BaseOverridableMeiliSearchSearchParameters = Pick<MeilisearchMultiSearchParams, 'sort' | 'hitsPerPage' | 'filter' | 'facets' | 'attributesToCrop' | 'attributesToRetrieve' | 'attributesToSearchOn' | 'cropLength' | 'cropMarker' | 'attributesToHighlight' | 'distinct' | 'highlightPreTag' | 'highlightPostTag' | 'hybrid' | 'matchingStrategy' | 'rankingScoreThreshold' | 'showMatchesPosition' | 'showRankingScore' | 'vector'>;
export type OverridableMeiliSearchSearchParameters = BaseOverridableMeiliSearchSearchParameters & {
    indexesOverrides?: Record<string, BaseOverridableMeiliSearchSearchParameters>;
};
...
export type InstantMeiliSearchOptions = Pick<MeilisearchConfig, 'requestInit' | 'httpClient'> & BaseInstantMeiliSearchOptions;
...
export type MeilisearchSearchResponse<T = Record<string, any>> = Omit<import('@algolia/client-search').SearchResponse<T>, 'hits'> & {
    hits: Array<T & {
        __position: number;
    }>;
    _meilisearch?: {
...
export type InstantMeiliSearchInstance = InstantSearchOptions['searchClient'] & {
    clearCache: () => void;
};
